...
import { executeWorkforceProfileCommand } from "../../../../../../lib/hr-workforce-profile-manage";
import {
...
function statusFor(error: unknown) {
  ...
  if (error.kind === "principal_unavailable") return 422;
...
export async function POST(request: Request) {
  ...
  const validation = validateWorkforceManagementCommand(payload);
  if (!validation.ok) {
    return response(
  ...
  try {
    const profile = await executeWorkforceProfileCommand(validation.command);
    return response(
      { ok: true, profile },
      validation.command.path.endsWith("profiles") ? 201 : 200,
    );
  } catch (error) {
