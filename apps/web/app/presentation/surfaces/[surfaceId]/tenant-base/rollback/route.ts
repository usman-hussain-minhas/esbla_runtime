...
import { rollbackTenantPresentationSurfaceBase } from "../../../../../../lib/presentation-surface-bases";
...
export async function POST(
  ...
  let input: ReturnType<typeof parsePresentationSurfaceBaseRollbackRequest>;
  let surfaceId: ReturnType<typeof parseZenV1SurfaceId>;
  try {
    surfaceId = parseZenV1SurfaceId((await context.params).surfaceId);
    input = parsePresentationSurfaceBaseRollbackRequest(await request.json());
  } catch {
    return json({ error: "invalid_input" }, 400);
  }
  try {
    return json(
      await rollbackTenantPresentationSurfaceBase(
        surfaceId,
        {
          expectedHeadRowVersion: input.expectedHeadRowVersion,
          sourceBaseVersion: input.sourceBaseVersion,
        },
